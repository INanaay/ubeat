<template>
  <div>
    <div v-if="!musicListMode">
      <div class="header">
        <h1>Playlists</h1>
        <button v-on:click="addPlaylist()" id="createButton">Create playlist</button>
      </div>
      <div id="playlists">
        <div v-for="playlist in playlists" v-bind:key="playlist.id">
            <div class="card"  v-bind:style="{'background-color': playlist.color}" >
              <button v-on:click="changeSelectState(playlist)" class="cardButton">
                <span v-bind:class="{blue: playlist.active}" class="mdi mdi-plus"></span>
              </button>
              ...
              <select v-if="playlist.active" size="10">
                <option v-for="song in fakeDB" v-bind:key="song.id" v-on:click="addSong(playlist, song)">
                 {{song.trackName}} - {{song.artistName}}
                </option>
              </select>
              <div v-if="!playlist.active" v-on:click="openPlaylist(playlist)" class="cardTitle">{{ playlist.name }}</div>
              <div v-if="!playlist.active" class="songSize">{{ playlist.size }} songs</div>
            </div>
        </div>
      </div>
    </div>
    <div v-else>
      <button class="backButton" v-on:click="backToPlaylists()"><span class="mdi mdi-undo"></span></button>
      <div class="header">
    ...
    export default {
        ...
        methods: {
                ...
                this.$prompt("New playlist name", name, "Rename playlist", "question")
                    .then(result => {
                        if (result.length > 10)
                            ...
                            this.$alert("Successfully renamed \"" + oldName + "\" to \"" + result + "\"",
                                "Renamed", "success")
                        }
                    })
                    .catch(() => {});
            ...
            addSong: function (playlist, song) {
                this.$confirm("Do you want to add \"" + song.trackName + "\" of \"" + song.artistName + "\"",
                    playlist.name, "question")
                    .then(result => {
                        if (result) {
                          playlist.addMusic(song);
                          playlist.active = false;
                            this.$alert("Successfully added new song",
                                playlist.name, "success")
                        }
                    })
            },
            deletePlaylist: function(playlist) {
                this.$confirm("Do you want to delete \"" + playlist.name + "\"", "Are you sure ?", "warning")
                    .then(result => {
                        if(result) {
                            db.removePlaylistBy(playlist.id);
                            this.$alert("Successfully deleted \"" + playlist.name + "\"", "Deleted", "success");
                        }
                    })
                    .catch(() => {});
            },
